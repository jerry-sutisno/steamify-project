...
Disusun Oleh:
Jerry Sutisno [202404026]
Muhamad Sarwan Al Barizy [202404013]
Umar Maula Sidiq [202404016]
...
1.1Latar Belakang5
1.2Lingkup Masalah5
1.3Definisi dan Istilah6
1.4Referensi7
BAB II  DESKRIPSI UMUM PERANGKAT LUNAK8
2.1Deskripsi Umum Sistem8
2.2Karakteristik Pengguna8
2.3Lingkungan Operasi9
2.4Batasan Sistem dan Asumsi9
BAB III  ANALISIS DAN PERANCANGAN10
3.1Analisa Kebutuhan Fungsional Perangkat Lunak10
3.1.1.Use Case Diagram10
3.1.2.Use Case Spesification10
3.1.3.Activity Diagram (belum di ubah)12
3.2Analisa Kebutuhan Non Fungsional16
3.3Perancangan16
3.3.1.Sequence Diagram16
3.3.2.Perancangan Data17
3.3.3.Perancangan Antarmuka Pengguna https://www.figma.com/design/R0dkCUHSWxbs1bg8hMqtwI/Untitled?node-id=0-1&t=60DaYZmpEsmkzi7G-118
DAFTRA GAMBAR
Gambar 1.Use Case10
Gambar 2.Activity Diagram Login13
Gambar 3. Activity Diagram Mengelola Data Barang14
Gambar 4. Activity Diagram Input Barang Masuk14
Gambar 5.Activity Diagram Input Barang Keluar15
Gambar 6. Monitoring Stok15
Gambar 7. Sequence Diagram Login16
Gambar 11.Perancangan Data17
BAB I PENDAHULUAN
Latar Belakang
...
Fokus Sistem Pemesanan (Booking): Sistem yang dikembangkan difokuskan pada manajemen pemesanan slot waktu operasional (hari dan jam) yang telah disediakan oleh pihak pengelola. Pemesanan ini dilakukan sepenuhnya melalui platform berbasis web.
Manajemen Hak Akses Pengguna (User Access): Sistem ini membatasi pengelompokan entitas pengguna menjadi dua peran (aktor) utama. Peran pertama adalah Pelanggan (Customer) yang memiliki akses untuk mendaftarkan data kendaraan pribadi dan membuat pesanan. Peran kedua adalah Admin (Pengelola) yang memiliki wewenang penuh untuk mengatur ketersediaan jadwal, paket harga layanan, memperbarui status pengerjaan, dan mengakses laporan transaksi harian.
Mekanisme Pembayaran di Muka (Pre-paid): Untuk mengikat antrean dan memastikan validitas pesanan, sistem menerapkan konsep pembayaran di awal. Pembayaran diakomodasi melalui pencatatan metode seperti QRIS, Transfer Bank, atau dompet digital (E-Wallet) di dalam antarmuka sistem.
Pelacakan Status Antrean Terpusat: Admin hanya bertugas memperbarui status dari setiap pesanan (mulai dari Pending, Dikonfirmasi, Diproses, hingga Selesai). Segala bentuk pembaruan status ini akan terefleksi secara langsung (real-time) di halaman Dashboard pelanggan tanpa memerlukan verifikasi manual tambahan.
...
Web Server: Apache HTTP Server atau Nginx.
...
Perangkat Lunak: Tidak membutuhkan instalasi aplikasi khusus. Pengguna hanya membutuhkan aplikasi peramban web (web browser) modern yang mendukung HTML5, CSS3 (Tailwind CSS), dan JavaScript, seperti Google Chrome, Mozilla Firefox, Apple Safari, atau Microsoft Edge.
Batasan Sistem dan Asumsi
...
Sistem bergantung sepenuhnya pada ketersediaan koneksi internet yang stabil, baik di pihak pelanggan saat melakukan pemesanan maupun di pihak admin saat mengelola antrean.
Modul pembayaran yang terdapat di dalam sistem saat ini bersifat simulasi pencatatan (dummy). Artinya, proses validasi pembayaran elektronik di dunia nyata masih dilakukan secara terpisah (misalnya melalui verifikasi kasir atau pengecekan mutasi rekening), dan sistem secara langsung mengasumsikan transaksi telah "Lunas" setelah tombol konfirmasi ditekan.
Sistem tidak menyediakan fitur penjadwalan ulang secara mandiri oleh pelanggan setelah pembayaran dikonfirmasi. Jika terjadi perubahan jadwal atau pembatalan, pelanggan diasumsikan harus menghubungi pihak pengelola secara langsung.
BAB III ANALISIS DAN PERANCANGAN
Analisa Kebutuhan Fungsional Perangkat Lunak
...
Pelanggan harus dapat melakukan registrasi, masuk (login), menambah data kendaraan, memesan jadwal (booking), memilih metode pembayaran, serta memantau riwayat transaksinya.
Administrator harus dapat mengelola paket layanan, mengatur ketersediaan slot waktu operasional, memperbarui status antrean kendaraan secara real-time, dan meninjau seluruh data transaksi yang masuk.
Use Case Diagram
Diagram Use Case berikut menggambarkan interaksi langsung antara Actor (Pelanggan dan Admin) dengan sistem Steamify secara keseluruhan.
Admin:
...
Table 1.Use Case Registrasi & Autentikasi
Atribut
Keterangan
Aktor Utama:
Pelanggan
Kondisi Awal:
Pelanggan membuka halaman Register atau Login namun belum masuk ke dalam sistem.
Alur Utama:
Pelanggan memasukkan data diri (Nama, Email, No HP, Password) pada formulir.
Pelanggan menekan tombol daftar/masuk.
Sistem memvalidasi kesesuaian kredensial atau ketersediaan email di database.
Sistem membuat sesi aktif untuk pengguna.
Kondisi Akhir:
Pelanggan berhasil masuk ke area Dasbor Pelanggan (Customer Dashboard).
Kelola Garasi (Data Motor):
Table 2. Use Case Spesification Kelola Garasi (Data Motor)
Atribut
Keterangan
Aktor Utama:
Pelanggan
Kondisi Awal:
Pelanggan telah berhasil login dan berada di halaman Vehicles (Kendaraan).
Alur Utama:
Pelanggan menekan tombol "Tambah Motor".
Pelanggan memasukkan atribut motor (Merk, Tipe, Warna, Plat Nomor).
Pelanggan menyimpan data formulir.
Sistem menyimpan data motor ke database dan menghubungkannya dengan ID Pelanggan tersebut.
Kondisi Akhir:
Data motor baru muncul di daftar garasi pelanggan dan siap dipilih saat booking.
Reservasi Jadwal (Booking):
Table 3. Use Case Spesification Reservasi Jadwal (Booking)
Atribut
Keterangan
Aktor Utama:
Pelanggan
Kondisi Awal:
Pelanggan memiliki minimal satu data motor dan berada di menu Booking.
Alur Utama:
Pelanggan menentukan tanggal kedatangan pada kalender.
Sistem memuat ketersediaan slot jam pada tanggal tersebut.
Pelanggan menyeleksi motor yang ingin dicuci, jadwal (slot waktu), dan paket layanan yang tersedia.
Sistem membuat rekaman draf pemesanan sementara dengan status Pending.
Kondisi Akhir:
Sistem mengarahkan layar pelanggan menuju halaman Checkout tagihan.
Checkout & Pembayaran:
Table 4. Use Case Spesification Checkout & Pembayaran
Atribut
Keterangan
Aktor Utama:
Pelanggan
Kondisi Awal:
Pelanggan memiliki tagihan pesanan berstatus Pending di halaman Checkout.
Alur Utama:
Pelanggan meninjau ulang total biaya dan layanan yang dipesan.
Pelanggan memilih satu dari empat simulasi metode pembayaran (Transfer Bank, QRIS, E-Wallet, Tunai).
Pelanggan menekan tombol "Bayar Sekarang".
Sistem mencatat pembayaran, mengunci slot waktu, mengubah status menjadi Dikonfirmasi, dan membuat nomor antrean.
Kondisi Akhir:
Transaksi dinyatakan lunas dan pelanggan mendapatkan nomor antrean resmi di dasbor.
Kelola Antrean Harian:
Table 5. Use Case Spesification Kelola Antrean Harian
Atribut
Keterangan
Aktor Utama:
Administrator (admin)
Kondisi Awal:
Admin berada di halaman Queue Management dan memilih tanggal operasional saat ini.
Alur Utama:
Admin melihat kartu pesanan yang masuk ke kolom Menunggu.
Saat motor pelanggan mulai dicuci, Admin menekan tombol "Proses".
Sistem memindahkan pesanan ke kolom Diproses secara otomatis.
Setelah motor selesai dicuci, Admin menekan tombol "Selesai" dan sistem memindahkannya ke kolom akhir.
Kondisi Akhir:
Status pengerjaan kendaraan terbarui menjadi Selesai dan langsung terefleksi di dasbor pelanggan yang bersangkutan.
Activity Diagram (belum di ubah)
Registrasi & Autentikasi:
Gambar 3 Registrasi & Autentikasi
Kelola Garasi:
Gambar 4 Kelola Garasi
Reservasi Jadwal (Booking):
    
Gambar 5 Jadwal Booking
Checkout & Pembayaran:
Gambar 6 Checkout & Pembayaran
Kelola Antrean:
Gambar 7 Kelola Antrean
...
Performance (Kinerja): Sistem harus mampu memuat (load) halaman dashboard dan alur pemesanan dalam waktu kurang dari 3 detik pada kondisi jaringan internet standar, guna mencegah pelanggan membatalkan transaksinya akibat loading yang terlalu lama.
Reliability (Keandalan): Sistem dituntut mampu mencegah terjadinya pemesanan jadwal secara ganda (Double Booking). Ketika seorang pelanggan mengunci slot jadwal melalui pembayaran, sistem harus mengamankan slot tersebut agar tidak bisa dibeli pelanggan lain.
Usability (Kemudahan Penggunaan): Antarmuka (UI) harus berprinsip Mobile-First, responsif, dan adaptif karena diasumsikan pelanggan lebih banyak mengakses layanan melalui telepon pintar.
Security (Keamanan): Sandi (Password) pengguna harus dienkripsi menggunakan metode hashing yang kuat (misalnya algoritma Bcrypt bawaan Laravel) dan mencegah adanya injeksi kode (SQL Injection/XSS) dari masukan pihak luar.
Perancangan
Sequence Diagram
Diagram sekuensi ini menjelaskan interaksi antar-objek pada saat Pelanggan mengonfirmasi pembayaran pesanannya.
Gambar 13. Sequence Diagram Login
Perancangan Data
Gambar 14.Perancangan Data
Penjelasan:
Struktur basis data (Database) pada sistem Steamify merupakan relasi dari berbagai entitas. Secara konseptual, skema tabel (Data Dictionary) yang dirancang mencakup:
Tabel users: Menyimpan data autentikasi baik untuk peran Pelanggan maupun Admin. Terdiri dari kolom id, name, email, password, dan penanda peran pengguna.
Tabel motor: Bertindak sebagai "Garasi Digital". Menyimpan atribut identitas kendaraan berupa id, id_pelanggan (Foreign Key), merk_motor, tipe_motor, warna_motor, dan plat_nomor.
Tabel paket_layanan: Master data untuk opsi pencucian (misalnya: Cuci Salju). Atributnya meliputi id, nama_paket, harga, dan estimasi_waktu.
Tabel jadwal: Mengelola ketersediaan waktu bengkel. Atribut utamanya adalah id, jam_slot, dan status_ketersediaan (Tersedia / Penuh).
Tabel booking: Tabel relasi inti yang menghubungkan pelanggan dengan jadwal dan layanannya. Terdiri dari kolom id, id_pelanggan, id_motor, id_paket, id_jadwal, tanggal_booking, status_booking, serta nomor_antrean.
Tabel transaksi: Tabel validasi finansial. Berisi id, id_booking (Foreign Key), nomor_struk, metode_bayar, tanggal_bayar, total_bayar, dan status_bayar.
